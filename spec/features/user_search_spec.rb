...
describe "Filter users by key", type: :feature do
    ...
    it "should be possible to filter by firstname" do
        visit users_search_path(firstname: "Ma")
        expect(page).to have_text("Max Mustermann")
        expect(page).not_to have_text("Grace")
    end

    it "should be possible to filter by lastname" do
       visit users_search_path(lastname: "mann")
       expect(page).to have_text("Max Mustermann")
       expect(page).not_to have_text("Grace")
    end

    it "should be possible to filter by username" do
        visit users_search_path(user: "Ma")
        expect(page).to have_text("Max Mustermann")
        expect(page).not_to have_text("Grace")
    end

    it "should be possible to filter by email" do
